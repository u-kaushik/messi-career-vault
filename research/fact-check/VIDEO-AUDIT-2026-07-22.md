...
| 2018–19 | Liverpool's 3:04 Anfield highlights; clear, high-contrast match graphic. Frames contain Origi's two goals, Wijnaldum's two and the quick corner. | Event description is accurate and no chapter photo is duplicated, but “Ninety seconds” incorrectly retains the former 1:29 clip's runtime framing. | **INCORRECT:** change the caption to “Three minutes of the night…” or remove the duration claim. |

## Automated publication gate

`npm run videos:check` currently passes all fifteen records. Static inspection confirms that `scripts/check-videos.mjs` exits non-zero when `yt-dlp` cannot resolve a video, when `availability` is not exactly `public`, when `playable_in_embed` is not exactly `True`, or when duration exceeds 720 seconds. `package.json` places this command first in the `deploy` chain with `&&`, so each of those failures prevents build/deployment.

The gate does **not** fully implement PROTOCOL.md § “Video selection and availability gate” item 7: the protocol explicitly makes an age-restricted result blocking, while the script neither prints nor tests `age_limit`. Every currently selected video reports `age_limit=0`, so this is a future-gate coverage defect rather than a defect in the present inventory. Required gate correction: include `%(age_limit)s` in the metadata output and fail when it is non-zero (or otherwise explicitly detect age restriction).
...
- Replacement-media review: **5 VERIFIED, 1 QUALIFIED, 1 INCORRECT**.
...
- Verdict: **HOLD** until the 2018–19 caption is synchronized with the 3:04 replacement and the age-restriction condition promised by PROTOCOL.md is enforced by the pre-deploy script. The 2012–13 clip is publishable but remains a visual-quality qualification.
